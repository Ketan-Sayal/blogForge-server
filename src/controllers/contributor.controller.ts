...
import { createContributor } from "../services/contributor.service.js";
...
export const signup = asyncHandler(async(req:Request, res:Response, _:NextFunction)=>{
    ...
    const user = await createContributor({username, email, password:hash, pic:picUrl, publicId:picPublicId});
    if(!user){
        throw new ApiError(500);
    }
    const token = jwt.sign({id:user.id}, config.jwtContributorSecret);
    return res.status(201).cookie("auth_contributor_token", token).json(new ApiResponse(201, {user, token}, "User sent successfully"));
});
...
export const signin = asyncHandler(async(req:Request, res:Response, _:NextFunction)=>{
    ...
    if(existingUser.role!==ROLES.CONTRIBUTOR){
        throw new ApiError(403, "User is not an contributor");
    }
    ...
    const token = jwt.sign({id:user.id}, config.jwtContributorSecret);
    return res.status(200).cookie("auth_contributor_token", token).json(new ApiResponse(200, {user, token}, "User logged in successfully"));
});