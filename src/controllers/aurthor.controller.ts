...
import { createAurthor } from "../services/aurthor.service.js";
...
export const signup = asyncHandler(async(req:Request, res:Response, _:NextFunction)=>{
    ...
    const user = await createAurthor({username, email, password:hash, pic:picUrl, publicId:picPublicId});
    if(!user){
        throw new ApiError(500);
    }
    const token = jwt.sign({id:user.id}, config.jwtAurthorSecret);
    return res.status(201).cookie("auth_aurthor_token", token).json(new ApiResponse(201, {user, token}, "User sent successfully"));
});
...
export const signin = asyncHandler(async(req:Request, res:Response, _:NextFunction)=>{
    ...
    if(existingUser.role!==ROLES.AURTHOR){
        throw new ApiError(403, "User is not an aurthor");
    }
    ...
    const token = jwt.sign({id:user.id}, config.jwtAurthorSecret);
    return res.status(200).cookie("auth_aurthor_token", token).json(new ApiResponse(200, {user, token}, "User logged in successfully"));
});